...
start_time=$(date +%s)
while true; do
  if curl -s -f -u elastic:${ELASTIC_PASSWORD} http://localhost:9200; then
    echo -e "\nElasticsearch is ready!"
    break
  else
    elapsed=$(( $(date +%s) - start_time ))
    if [ $elapsed -ge $timeout ]; then
      echo -e "\nERROR: Elasticsearch did not become ready within $timeout seconds"
      docker logs ragflow-es-01  # 输出 Elasticsearch 日志
      exit 1
    fi
    echo "Elasticsearch is unavailable - retrying in 10 seconds... (已等待 ${elapsed} 秒)"
    sleep 10
  fi
done
